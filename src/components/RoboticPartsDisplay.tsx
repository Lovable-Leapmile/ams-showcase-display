  ...
  return <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex flex-col">
      ...
      <div className="p-8">
        <div className="flex justify-center space-x-4">
          {stations.map(station => <div key={station.id} className={cn("flex flex-col items-center space-y-2 transition-all duration-300", currentStation === station.id ? "scale-110" : "scale-100")}>
              {/* Station Indicator */}
              <div className={cn("w-16 h-16 rounded-lg flex items-center justify-center text-2xl font-bold transition-all duration-300 border-2", currentStation === station.id ? "bg-teal-600 text-white border-teal-400 shadow-lg shadow-teal-500/50" : station.parts.length > 0 ? "bg-slate-600 text-white border-slate-500 hover:bg-slate-500" : "bg-slate-800 text-slate-500 border-slate-700")}>
                {station.id}
              </div>
              
              {/* Parts Count Indicator */}
              <div className="text-center">
                <div className={cn("text-sm font-medium", currentStation === station.id ? "text-teal-400" : station.parts.length > 0 ? "text-slate-300" : "text-slate-600")}>
        ...
        {/* Progress Indicators */}
        {displayPart && <div className="mt-6 flex justify-center">
